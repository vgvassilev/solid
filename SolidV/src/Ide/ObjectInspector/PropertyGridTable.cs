...
namespace SolidV.Gtk.InspectorGrid
{
  ...
  class InspectorGridTable: Gtk.EventBox
  {
    ...
    const int InspectorTopBottomPadding = 5;
    const int InspectorLeftPadding = 8;
    const int InspectorContentLeftPadding = 8;
    const int InspectorIndent = 8;
    ...
    class TableRow
    {
      ...
      public PropertyDescriptor Inspector;
    ...
    public InspectorGridTable(EditorManager editorManager, InspectorGrid parentGrid)
    {
...
//      discloseUp = Gdk.Pixbuf.LoadFromResource("disclose-arrow-up.png");
    ...
    public InspectorSort InspectorSort { get; set; }
    ...
    {
      bool categorised = InspectorSort == InspectorSort.Categorized;
      ...
      if(!categorised) {
        ...
        foreach(PropertyDescriptor pd in sorted) {
          ...
          AppendInspector(rowList, pd, instance);
        }
      }
      QueueDraw();
      QueueResize();
    ...
    internal void Update(PropertyDescriptorCollection properties, object instance)
    {
      foreach(PropertyDescriptor pd in properties)
        UpdateInspector(pd, instance, rows);
      QueueDraw();
      QueueResize();
    }
    
    bool UpdateInspector(PropertyDescriptor pd, object instance, IEnumerable<TableRow> rowList)
    {
      foreach(var row in rowList) {
        if(row.Inspector != null && row.Inspector.Name == pd.Name && row.Instace == instance) {
          row.Inspector = pd;
          return true;
        }
        if(row.ChildRows != null) {
          if(UpdateInspector(pd, instance, row.ChildRows))
            return true;
        }
      }
      return false;
    ...
    void AppendInspector(PropertyDescriptor prop, object instance)
    {
      AppendInspector(rows, prop, new InstanceData(instance));
    }
    
    void AppendInspector(List<TableRow> rowList, PropertyDescriptor prop, object instance)
    {
      TableRow row = new TableRow() {
        IsCategory = false,
        Inspector = prop,
      ...
      if(typeof(ExpandableObjectConverter).IsAssignableFrom(tc.GetType())) {
        ...
        foreach(PropertyDescriptor cprop in TypeDescriptor.GetProperties(cob))
          AppendInspector(row.ChildRows, cprop, cob);
      }
    }

    InspectorEditorCell GetCell(TableRow row)
    {
      var e = editorManager.GetEditor(row.Inspector);
      e.Initialize(this, editorManager, row.Inspector, row.Instace);
      return e;
    }
    ...
    protected override void OnSizeRequested(ref Requisition requisition)
    {
      ...
      int dx =(int)((double)Allocation.Width * dividerPosition) - InspectorContentLeftPadding;
      if(dx < 0) dx = 0;
      int y = 0;
    ...
    void MeasureHeight(IEnumerable<TableRow> rowList, ref int y)
    {
      ...
      foreach(var r in rowList) {
        ...
        if(r.IsCategory) {
          ...
          eh = Math.Max(h + InspectorTopBottomPadding * 2, eh);
          r.EditorBounds = new Gdk.Rectangle(dividerX + InspectorContentLeftPadding, y, Allocation.Width - dividerX - InspectorContentLeftPadding, eh);
          y += eh;
        }
...
//        var s = "<b>" + row.Inspector.DisplayName + "</b>\n\n";
//        s += GLib.Markup.EscapeText(row.Inspector.Description);
